# extracted from https://github.com/google/prompt-to-prompt/blob/main/prompt-to-prompt_stable.ipynb
import abc
import torch
import numpy as np
...
class LocalBlend:
    ...
    def __call__(self, x_t, attention_store):
        ...
        mask = (mask[:1] + mask[1:]).float().to(x_t.dtype)
...
class AttentionReplace(AttentionControlEdit):
    ...
    def __init__(self, tokenizer, prompts, num_steps: int, cross_replace_steps: float, self_replace_steps: float,
                 local_blend: Optional[LocalBlend] = None, device='cpu'):
...
class AttentionReweight(AttentionControlEdit):
    ...
    def __init__(self, tokenizer, prompts, num_steps, cross_replace_steps, self_replace_steps, equalizer,
                 local_blend, controller, device):
